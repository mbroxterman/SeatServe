      ...
      {filteredRunners.length === 0 ? (
        ...
        <div className="runner-grid">
          {filteredRunners.map((runner) => {
            ...
            return (
              <article key={runner.id} className={`runner-card ${!runner.active ? "is-inactive" : ""}`}>
                ...
                <div className="runner-card__status-controls" aria-label={`Set ${runner.name} status`}>
                  {(["available", "assigned", "offline"] as RunnerStatus[]).map((status) => (
                    <button
                      key={status}
                      type="button"
