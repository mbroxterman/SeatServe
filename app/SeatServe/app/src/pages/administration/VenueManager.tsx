...
export default function VenueManager() {
  const { data, activeEvent, addVenue, updateVenue, deleteVenue, addZone, updateZone, deleteZone } = useSeatServe();
  ...
  const customerPath = (venueId: string, zoneId: string) => {
    const event = data.events.find((item) => item.venueId === venueId && item.orderingEnabled) ?? activeEvent;
    return event ? `/order/${event.id}/${venueId}/${zoneId}` : "";
  };

  const openZone = (venueId: string, zoneId: string) => {
    const path = customerPath(venueId, zoneId);
    if (!path) return setNotice("Open customer ordering for an event before testing a zone QR link.");
    window.open(path, "_blank", "noopener,noreferrer");
  };

  const copyZone = async (venueId: string, zoneId: string) => {
    const path = customerPath(venueId, zoneId);
    if (!path) return setNotice("Open customer ordering for an event before copying a zone link.");
