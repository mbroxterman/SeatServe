      ...
      <section className="ko-workspace">
        <div className="ko-operations-bar">
          ...
          <div className="ko-toolbar">
            ...
            <button type="button" className="ko-tool ko-tool--primary"><Zap size={17}/>Auto Assign<ChevronDown size={14}/></button>
            <button type="button" className="ko-tool"><Printer size={17}/>Print Tickets<ChevronDown size={14}/></button>
            <button type="button" className="ko-tool ko-tool--icon" aria-label="Filter orders"><SlidersHorizontal size={18}/></button>
          ...
          {columns.map((column) => (
            <section className={`ko-column ko-column--${column.key}`} key={column.key}>
              ...
              <div className="ko-column__list">
                {column.orders.length === 0 ? <div className="ko-column__empty">No orders</div> : column.orders.slice(0, 6).map((order) => (
                  <OrderCard
                    key={order.id}
                    order={order}
                    zone={zoneName(order)}
                ...
                {column.orders.length > 6 && <button className={`ko-more ko-more--${column.key}`} type="button">+ {column.orders.length - 6} more orders</button>}
              </div>
            </section>
          ))}
        </main>
...
function OrderDetail({ order, zone, runner, runners, onClose, onStatus, onAssign, onAutoAssign, onCancel }: { order: Order; zone: string; runner?: Runner; runners: Runner[]; onClose: () => void; onStatus: (status: OrderStatus) => void; onAssign: (runnerId?: string) => void; onAutoAssign: () => void; onCancel: () => void }) {
  ...
  const next = order.status === "new" ? { label: "Accept Order", status: "preparing" as OrderStatus }
    : order.status === "preparing" ? { label: "Mark Ready", status: "ready" as OrderStatus }
      : order.status === "assigned" ? { label: "Runner Picked Up", status: "delivering" as OrderStatus }
        : order.status === "delivering" ? { label: "Mark Delivered", status: "delivered" as OrderStatus }
          : undefined;
  ...
  return <div className="ko-detail__content">
    ...
    <section className="ko-runner"><h3>Runner</h3><div className="ko-runner__row"><span className="ko-avatar">{runner?.name?.slice(0, 1) ?? "?"}</span><div><strong>{runner?.name ?? "Unassigned"}</strong><small>{runner?.status ? titleCase(runner.status) : "No runner assigned"}</small></div>{order.status === "ready" ? <button type="button" onClick={onAutoAssign}>Auto Assign</button> : (order.status === "assigned" || order.status === "delivering") ? <select value={order.runnerId ?? ""} onChange={(event) => onAssign(event.target.value || undefined)}><option value="">Unassigned</option>{availableRunners.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : null}</div></section>

    <section className="ko-total"><span>Order Total</span><strong>${Number(order.total ?? 0).toFixed(2)}</strong></section>

    <footer className="ko-detail__footer">{next && <button type="button" className="ko-primary-action" onClick={() => onStatus(next.status)}><Check size={18}/>{next.label}</button>}<div><button type="button" className="ko-secondary-action"><CirclePause size={17}/>Hold Order</button>{order.status !== "delivered" && order.status !== "cancelled" && <button type="button" className="ko-danger-action" onClick={onCancel}><X size={17}/>Cancel Order</button>}</div></footer>
  </div>;
}
