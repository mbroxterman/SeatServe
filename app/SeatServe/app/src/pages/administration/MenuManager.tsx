...
import { CirclePlus, Eye, EyeOff, GripVertical, ImagePlus, Pencil, Search, Trash2, UtensilsCrossed, X } from "lucide-react";
...
export default function MenuManager() {
  const { data, addMenuCategory, updateMenuCategory, reorderMenuCategories, reorderMenuItems, deleteMenuCategory, addMenuItem, updateMenuItem, deleteMenuItem, addMenu, updateMenu, deleteMenu, assignMenuToEvent } = useSeatServe();
  ...
  return <section className="menu-page">
    ...
    {activeTab === "assignments" && <section className="menu-assignments"><header><h2>Event menu assignments</h2><p>The QR code identifies the event; the event automatically selects its assigned menu.</p></header><div>{data.events.map((event) => <article key={event.id}><div><strong>{event.name}</strong><span>{event.opponent ? `vs. ${event.opponent}` : ""}</span><small>{new Date(event.startsAt).toLocaleString()}</small></div><label>Menu<select value={event.menuId ?? ""} onChange={(change) => assignMenuToEvent(event.id, change.target.value || undefined)}><option value="">No menu assigned</option>{data.menus.filter((menu) => menu.active).map((menu) => <option key={menu.id} value={menu.id}>{menu.name}</option>)}</select></label></article>)}</div></section>}
