...
export interface Order { id:string; eventId:string; runnerId?:string; items:OrderLineItem[]; customer:OrderCustomer; location:OrderLocation; subtotal:number; tax:number; total:number; deliveryFee:number; paymentMethod?:PaymentMethod; cashTotal?:number; estimatedCardFee?:number; cardTotal?:number; status:OrderStatus; placedAt:string; acceptedAt?:string; preparingAt?:string; readyAt?:string; assignedAt?:string; deliveringAt?:string; deliveredAt?:string; assignmentQueuedAt?:string; }
...
export interface SeatServeData { events:SeatServeEvent[]; venues:Venue[]; runners:Runner[]; menuCategories:MenuCategory[]; menuItems:MenuItem[]; menus:MenuDefinition[]; orders:Order[]; activity:ActivityItem[]; customerExperience:CustomerExperienceSettings; feedback:CustomerFeedback[]; }
