...
export function SeatServeProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        const loadFromSheets = async (isAutoLoad = false) => {
            ...
            try {
                ...
                if (!isAutoLoad) window.alert(Load failed.Could not get data from Google Sheets.${ error instanceof Error ? error.message : "Using local data instead." });
            }
        };
        loadFromSheets(true);
    }, [workspaceRevision]);
    ...
    useEffect(() => {
        ...
        if (typeof BroadcastChannel !== "undefined") { const channel = new BroadcastChannel("seatserve-live-data-v1"); channelRef.current = channel; channel.onmessage = (event: MessageEvent<{ sourceId?: string; workspaceId?: string; data?: SeatServeData }>) => { if (event.data?.sourceId === instanceIdRef.current) return; if (event.data?.workspaceId !== getActiveWorkspaceId() || !event.data?.data) return; applyExternalData(event.data.data); }; } const refreshFromLocalStorage = () => {
            try { const raw = localStorage.getItem(getActiveDataStorageKey()); if (raw) applyExternalData(JSON.parse(raw) as SeatServeData); } catch { // Keep the current in-memory state if another tab writes incomplete data. } }; const onStorage = (event: StorageEvent) => { if (event.key === getActiveDataStorageKey() && event.newValue) refreshFromLocalStorage(); }; const onWorkspaceSwitch = () => { try { const raw = localStorage.getItem(getActiveDataStorageKey()); replacingDataRef.current = true; setData(raw ? migrateData(JSON.parse(raw) as SeatServeData) : migrateData(seedData)); } catch { replacingDataRef.current = true; setData(migrateData(seedData)); } setWorkspaceRevision((value) => value + 1); }; const onSyncConfig = () => setWorkspaceRevision((value) => value + 1); const onVisibility = () => { if (document.visibilityState === "visible") refreshFromLocalStorage(); }; window.addEventListener("storage", onStorage); window.addEventListener("seatserve:workspace-switched", onWorkspaceSwitch); window.addEventListener("seatserve:sync-config", onSyncConfig); window.addEventListener("focus", refreshFromLocalStorage); document.addEventListener("visibilitychange", onVisibility); return () => { channelRef.current?.close(); channelRef.current = null; window.removeEventListener("storage", onStorage); window.removeEventListener("seatserve:workspace-switched", onWorkspaceSwitch); window.removeEventListener("seatserve:sync-config", onSyncConfig); window.removeEventListener("focus", refreshFromLocalStorage); document.removeEventListener("visibilitychange", onVisibility); };
            }, []);
    ...
    useEffect(() => {
        ...
        currentDataRef.current = data; const serialized = JSON.stringify(data); serializedDataRef.current = serialized; try { localStorage.setItem(getActiveDataStorageKey(), serialized); } catch (error) { console.error("SeatServe local storage save failed", error); window.dispatchEvent(new CustomEvent("seatserve:storage-error", { detail: { message: "SeatServe could not save this change locally. Remove or replace large menu images and try again." } })); return; } channelRef.current?.postMessage({ sourceId: instanceIdRef.current, workspaceId: getActiveWorkspaceId(), data }); if (!didMountRef.current) { didMountRef.current = true; } else if (replacingDataRef.current) { replacingDataRef.current = false; } else { markLocalChange(); const config = getSyncConfig(); if (config.autoSync && config.connected && config.endpointUrl.trim() && navigator.onLine) { const delayMs = Math.max(0, config.autoSyncIntervalSeconds) * 1000; autoSyncTimerRef.current = window.setTimeout(() => { if (!cancelled) void pushToGoogleSheets(data).catch(() => undefined); }, delayMs); } } return () => { cancelled = true; window.clearTimeout(autoSyncTimerRef.current); autoSyncTimerRef.current = undefined; };
    }, [data]);
...
const updateOrderStatus = (orderId: string, status: Order["status"]) => {
    setData((current) => {
        ...
        const allowedTransitions: Record<Order["status"], Order["status"][]> = { new: ["preparing", "cancelled"], preparing: ["ready", "cancelled"], ready: order.fulfillmentMethod === "pickup" ? ["delivered", "cancelled"] : ["assigned", "cancelled"], assigned: ["delivering", "ready", "cancelled"], delivering: ["delivered", "cancelled"], delivered: [], cancelled: [], }; if (!allowedTransitions[order.status].includes(status)) { return { ...current, activity: pushActivity(current, `Blocked invalid order transition ${order.id}: ${order.status} → ${status}`, "warning") }; } if (status === "delivering" && order.fulfillmentMethod !== "pickup" && !order.runnerId) { return { ...current, activity: pushActivity(current, `Order ${order.id} cannot leave the kitchen without an assigned runner`, "warning") }; } const requiresPayment = order.paymentMethod === "cash" || order.paymentMethod === "card"; if (status === "delivered" && order.fulfillmentMethod !== "pickup" && requiresPayment && !order.paymentCollectedAt) { return { ...current, activity: pushActivity(current, `Order ${order.id} cannot be delivered until payment is collected`, "warning") }; } const now = new Date().toISOString(); const timestampPatch = status === "preparing" ? { acceptedAt: order.acceptedAt ?? now, preparingAt: order.preparingAt ?? now } : status === "ready" ? { readyAt: order.readyAt ?? now } : status === "assigned" ? { assignedAt: order.assignedAt ?? now } : status === "delivering" ? { deliveringAt: order.deliveringAt ?? now } : status === "delivered" ? { deliveredAt: order.deliveredAt ?? now, ...(order.fulfillmentMethod === "pickup" && requiresPayment && !order.paymentCollectedAt ? { paymentCollectedAt: now } : {}) } : {}; const runnerAfterStatus = current.runners.map((runner) => { if (runner.id !== order.runnerId) return runner; if (status === "delivered") return { ...runner, status: "returning" as const, activeOrderId: orderId }; if (status === "cancelled" || status === "ready") return { ...runner, status: "available" as const, activeOrderId: undefined, assignedAt: undefined, estimatedAvailableAt: undefined, availableSince: now }; if (status === "delivering") return { ...runner, status: "assigned" as const, activeOrderId: orderId }; return runner; }); return { ...current, orders: current.orders.map((item) => item.id === orderId ? { ...item, status, ...timestampPatch, ...(status === "ready" ? { runnerId: undefined, assignedAt: undefined } : {}) } : item), runners: runnerAfterStatus, activity: pushActivity(current, `Order ${orderId} moved to ${status}`, status === "cancelled" ? "warning" : status === "delivered" ? "success" : "info"), };
    });
};
    ...
    return (
        <main style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "#f4f7fb", padding: 24, fontFamily: "Inter, system-ui, sans-serif" }}>
            <section style={{ textAlign: "center", color: "#101d45" }}>
                ...
                <h1 style={{ margin: "18px 0 8px" }}>Loading SeatServe
                    <p style={{ margin: 0, color: "#66738f" }}>Connecting to the live workspace…
