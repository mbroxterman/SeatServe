...
export default function SettingsPage() {
    ...
    useEffect(() => { Promise.resolve().then(() => setExperience(data.customerExperience)); }, [data.customerExperience]);
    ...
    const runConnectionTest = async () => {
        setBusy(true); setMessage("");
        try {
            const result = await testGoogleSheetsConnection(config.endpointUrl, config.workspaceName || active.name);
            setMeta(getSyncMeta());
            setMessage(`Connection test successful${result.schemaVersion ? ` · Apps Script schema v${result.schemaVersion}` : ""}.`);
        } catch (error) {
            setMeta(getSyncMeta());
            setMessage(error instanceof Error ? `Connection test failed: ${error.message}` : "Connection test failed.");
        } finally { setBusy(false); }
    };
        ...
        <section className="settings-page">
            ...
            <div className="settings-grid">
                <article className="settings-card settings-card--wide workspace-card">
                    ...
                    {connected && <div className="sync-preferences">{deploymentManaged ? <div className="settings-toggle"><CheckCircle2 size={18} /><span><strong>Automatic synchronization enabled</strong><small>Production changes sync immediately; other devices check for updates automatically.</small></span></div> : <><label className="settings-toggle"><input type="checkbox" checked={config.autoSync} onChange={(event) => setConfig({ ...config, autoSync: event.target.checked })} /><span><strong>Automatic synchronization</strong><small>Sync only this workspace when its data changes.</small></span></label><label className="settings-field settings-field--compact"><span>Sync delay</span><select value={config.autoSyncIntervalSeconds} disabled={!config.autoSync} onChange={(event) => setConfig({ ...config, autoSyncIntervalSeconds: Number(event.target.value) as AutoSyncInterval })}><option value={0}>Immediately</option><option value={30}>30 seconds</option><option value={60}>1 minute</option><option value={300}>5 minutes</option></select></label></>}<div className="settings-actions">{!deploymentManaged && <button onClick={savePreferences}><Save size={17} /> Save preferences</button>}<button className="secondary" disabled={busy} onClick={() => void runConnectionTest()}><RefreshCw size={17} /> Test connection</button><button className="secondary" disabled={busy} onClick={() => runPush(false)}><CloudUpload size={17} /> Sync now</button><button className="secondary" disabled={busy} onClick={runPull}><CloudDownload size={17} /> Load from Sheets</button></div></div>}
                    {connected && <div className={`sync-diagnostics${meta.state === "error" ? " is-error" : meta.state === "saved" ? " is-healthy" : ""}`}><div className="sync-diagnostics__title"><Database size={18} /><div><strong>Connection diagnostics</strong><small>{deploymentManaged ? "Netlify-managed production endpoint" : "Workspace Apps Script endpoint"}</small></div></div><div className="sync-diagnostics__grid"><div><span>Current state</span><strong>{meta.state === "saved" ? "Healthy" : meta.state === "error" ? "Failed" : meta.state === "saving" ? "Testing / syncing" : meta.state === "conflict" ? "Conflict" : "Ready"}</strong></div><div><span>Last attempt</span><strong>{meta.lastAttemptAt ? new Date(meta.lastAttemptAt).toLocaleString() : "Not recorded"}</strong></div><div><span>Last successful sync</span><strong>{meta.lastSuccessfulSyncAt ? new Date(meta.lastSuccessfulSyncAt).toLocaleString() : "None yet"}</strong></div><div><span>Action</span><strong>{meta.lastAttemptAction ?? "—"}</strong></div><div><span>HTTP status</span><strong>{meta.lastHttpStatus ?? "—"}</strong></div><div><span>Response time</span><strong>{meta.lastResponseMs !== undefined ? `${meta.lastResponseMs} ms` : "—"}</strong></div><div><span>Apps Script schema</span><strong>{meta.lastSchemaVersion !== undefined ? `v${meta.lastSchemaVersion}` : "—"}</strong></div><div><span>Remote workspace</span><strong>{meta.remoteWorkspaceName || "—"}</strong></div></div>{meta.lastError && <div className="sync-diagnostics__error"><AlertCircle size={17} /><div><strong>Last error</strong><span>{meta.lastError}</span></div></div>}</div>}
            ...
            <article className="settings-card settings-card--wide checkout-payments-settings">
                ...
                <div className="checkout-settings-grid">
                    {/* I've added the new field here */}
                    <label className="settings-field">
                        <span>Minimum order amount ($)</span>
                        <input type="number" min="0" step="0.01" inputMode="decimal" value={experience.minimumOrderAmount ?? 0} onChange={(event) => setExperience({ ...experience, minimumOrderAmount: Math.max(0, Number(event.target.value) || 0) })} />
                        <small>Requires cart subtotal to meet this value.</small>
                    </label>
...
function SyncStatus({ meta, connected }: { meta: SyncMeta; connected: boolean }) {
    ...
    return <div className={`sync-status sync-status--${meta.state}${meta.pendingChanges ? " is-pending" : ""}`}>{meta.state === "error" || meta.state === "conflict" ? <AlertCircle size={17} /> : <CheckCircle2 size={17} />}<span><strong>{label}</strong>{meta.state === "error" && meta.lastError ? <small title={meta.lastError}>{meta.lastError}</small> : meta.lastSuccessfulSyncAt && <small>Last sync {new Date(meta.lastSuccessfulSyncAt).toLocaleString()}</small>}</span></div>;
}
