...
export default function CustomerExperience() {
    ...
    const entries = useMemo<QrEntry[]>(() => {
        const staff: QrEntry = { id: "staff", kind: "staff", title: "SeatServe Staff", subtitle: "Kitchen · Runner · Administration", url: `${origin}/staff?w=${encodeURIComponent(activeWsName)}`, filename: "seatserve-staff-qr.png" };
        const zones = data.venues.filter(v => v.active).flatMap(v => v.zones.filter(z => z.active && z.deliveryEnabled).map(z => ({ id: `zone-${v.id}-${z.id}`, kind: "zone" as const, title: z.name, subtitle: v.name, url: `${origin}/order/zone/${slug(v.name)}/${slug(z.name)}?w=${encodeURIComponent(activeWsName)}`, filename: `seatserve-${safeFile(v.name)}-${safeFile(z.name)}-qr.png`, venueName: v.name })));
        return [staff, ...zones];
    }, [data.venues, origin, activeWsName]);
