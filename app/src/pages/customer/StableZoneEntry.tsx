...
export default function StableZoneEntry(){
  ...
  useEffect(()=>{ let alive=true; loadCustomerBootstrap().then(result=>{ if(alive&&result.ok&&result.data) replaceData({ ...data, ...result.data }, "Customer QR cloud bootstrap"); }).catch(()=>{}).finally(()=>alive&&setLoading(false)); return()=>{alive=false}; },[]);
  ...
  if(loading && (!venue || !zone)) return <main style={{minHeight:"100vh",display:"grid",placeItems:"center",fontFamily:"Inter,system-ui,sans-serif"}}><h2>Loading SeatServe location…</h2></main>;
