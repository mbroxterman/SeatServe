...
export default function CustomerOrder() {
    ...
    const submit = async () => {
        ...
        const orderId = placeOrder(payload);
        localStorage.removeItem(storageKey);

        // Allow React 150ms to save the new order into its memory before we push
        ...
        try {
            ...
            console.error("Instant push failed, but order is saved locally.", error);
        }
                    ...
                    <div className="compact-menu-list">
                        {groupedItems.map(([group, items]) => {
                            ...
                            return <section className="compact-menu-group" key={group}>
                                <button className="compact-menu-group__header" onClick={() => setExpandedGroups((current) => ({ ...current, [group]: !expanded }))}><span><strong>{group}</strong><small>{items.length} choices</small></span>{expanded ? <ChevronUp size={20} /> : <ChevronDown size={20} />}</button>
                                {expanded && <div className="compact-menu-group__items">{items.map((item) => {
                                    ...
                                    return <article className={`compact-menu-row ${!item.available ? "is-sold-out" : ""}`} key={item.id}>
                                        <div className="compact-menu-row__visual">{item.imageUrl && item.displayStyle !== "emoji" ? <img src={item.imageUrl} alt={item.imageAlt || item.name} /> : <span>{item.emoji || categoryIcon(item.category)}</span>}</div>
