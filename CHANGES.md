...
## Multi-order pickup (new feature)
A runner can now carry up to 3 orders at once, self-service - they pick
additional ready orders from a list on their own screen rather than Kitchen
batch-assigning them. Design decisions made with the client: no restriction
on mixing zones in one run, capped at 3 orders, and payment/SeatBeacon/
delivered are still tracked per order, not per batch.

**This requires BOTH a Code.gs redeploy AND a Netlify frontend deploy** - it
touches the Runners sheet schema (activeOrderId -> activeOrderIds, comma-
separated like zoneIds) as well as the app.

- **types/domain.ts**: `Runner.activeOrderId?: string` is now
  `Runner.activeOrderIds: string[]`.
- **google-apps-script/Code.gs**: Runners sheet column renamed to
  activeOrderIds (comma-separated). Reads fall back to the old activeOrderId
  column for any not-yet-migrated rows, so existing data isn't lost on first
  read after redeploying - just redeploy once and it self-migrates. Updated
  assignRunner_, autoAssignRunner_, runnerStatus_, and markRunnerAvailable_
  for array semantics and the 3-order cap (MAX_RUNNER_ORDERS constant).
- **state/SeatServeContext.tsx**: updateOrderStatus, assignRunnerToOrder,
  autoAssignRunner, markRunnerAvailable, and setRunnerStatus all rewritten
  for the array. A runner only becomes "returning" (prompting the back-at-
  kitchen button) once every order they're carrying is delivered - not just
  one. Learned round-trip-time estimates now update per zone per order, since
  a run can span multiple zones.
- **pages/runner/RunnerMobile.tsx**: shows one card per active order instead
  of a single assignment, a single "I'm back at the kitchen" button once
  everything's delivered (not per order), and a new "Add to your run" list
  of ready orders when the runner has spare capacity.
- **Found and fixed two related latent bugs** while touching this code: both
  RunnerManager's "stuck on deleted order" check and the Settings page's
  workspace-health repair tool were incorrectly treating a runner's
  legitimately-delivered-but-not-yet-confirmed order as if it were stale/
  broken. Both now correctly treat "delivered" as a valid state, only
  flagging genuinely nonexistent order references.

**Not yet tested end to end** - this is a big change (schema + backend +
UI). Recommend a careful test pass before relying on it live: assign/pick up
2-3 orders on one runner, delivering them in different orders, confirming
payment and SeatBeacon work independently per order, and confirming "back at
the kitchen" only appears once everything is delivered.
